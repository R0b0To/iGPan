...
Future<Session> login(String email, String password, {bool isRetry = false}) async {
  ...
  /// Clear session metadata and cookies for [email].
...
Future<Session> reLogin(String email) async {
  ...
  // 2. A small delay is crucial. It gives the CookieJar/FileSystem 
  // time to release file locks and clear memory.
  await Future.delayed(const Duration(milliseconds: 300));

  // 3. Call login with isRetry: true to tell it we already cleaned the cookies
  return login(email, password, isRetry: true);
}
