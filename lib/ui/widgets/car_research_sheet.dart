...
class CarResearchSheet extends ConsumerStatefulWidget {
  ...
  const CarResearchSheet({super.key, required this.carData, required this.accountEmail});
...
class _CarResearchSheetState extends ConsumerState<CarResearchSheet> {
  ...
  int get _dpSpent => widget.carData.attributes.fold(0, (s, a) => s + max(0, (_designValues[a.key] ?? a.baseValue) - a.baseValue));
  int get _dpRemaining => widget.carData.designPoints - _dpSpent;
  bool get _hasDesignChanges => _dpSpent > 0;
  bool get _hasResearchChanges => _originalResearch.length != _selectedResearch.length || _originalResearch.any((k) => !_selectedResearch.contains(k));
  bool get _canSave => (_hasResearchChanges && _selectedResearch.isNotEmpty) || _hasDesignChanges;

  String get _saveLabel {
    final parts = [if (_hasResearchChanges && _selectedResearch.isNotEmpty) 'Research', if (_hasDesignChanges) 'Design'];
    return parts.isEmpty ? 'No changes' : 'Save ${parts.join(' & ')}';
  }

  String? get _saveSublabel {
    final parts = [
      if (_hasResearchChanges && _selectedResearch.isNotEmpty) '${_selectedResearch.length} attr${_selectedResearch.length == 1 ? "" : "s"}',
  ...
  int _estimatedGain(CarAttribute attr) {
    ...
    if (n == 0 || !_selectedResearch.contains(attr.key) || attr.leagueMax <= 0 || attr.baseValue >= attr.leagueMax) return 0;
  ...
  void _toggleResearch(String key) => setState(() => _selectedResearch.contains(key) ? _selectedResearch.remove(key) : _selectedResearch.add(key));
  void _increment(String k) { if (_dpRemaining > 0 && _designValues[k]! < widget.carData.dMax) setState(() => _designValues[k] = _designValues[k]! + 1); }
  void _decrement(String k) { if (_designValues[k]! > widget.carData.attributeByKey(k)!.baseValue) setState(() => _designValues[k] = _designValues[k]! - 1); }
  ...
  Future<void> _save() async {
    ...
    if (_hasResearchChanges && _selectedResearch.isNotEmpty) {
      try { await ref.read(carServiceProvider).submitResearch(widget.accountEmail, leagueId: widget.carData.researchLeagueId, attributes: _selectedResearch.toList()); }
      catch (e) { errors.add('Research: $e'); }
    }
    if (_hasDesignChanges) {
      try { await ref.read(carServiceProvider).submitDesign(widget.accountEmail, carId: widget.carData.carDesignId, leagueId: widget.carData.designLeagueId, attributeValues: Map.from(_designValues)); }
      catch (e) { errors.add('Design: $e'); }
    }
    ...
    if (mounted) {
      if (errors.isEmpty) {
        ...
        final saved = [if (_hasResearchChanges && _selectedResearch.isNotEmpty) 'Research', if (_hasDesignChanges) 'Design'];
      ...
      child: Column(
        ...
        children: [
          ...
          ...car.attributes.map((attr) {
            ...
            return _AttrRow(
              attr: attr, designValue: designVal, dpDelta: designVal - attr.baseValue,
              isSelected: _selectedResearch.contains(attr.key), estimGain: _estimatedGain(attr),
              canInc: canInc, canDec: designVal > attr.baseValue,
              onToggle: (attr.isAtLeagueMax && !_selectedResearch.contains(attr.key)) ? null : () => _toggleResearch(attr.key),
          ...
          SizedBox(
            ...
            child: AnimatedContainer(
              ...
              child: Material(
                ...
                child: InkWell(
                  borderRadius: BorderRadius.circular(10), onTap: (_canSave && !_saving) ? _save : null,
                  child: Padding(
                    ...
                    child: _saving
                      ...
                      : Column(mainAxisSize: MainAxisSize.min, children: [
                          ...
                          const SizedBox(height: 2),
                          Visibility(
                            visible: _saveSublabel != null, maintainSize: true, maintainAnimation: true, maintainState: true,
...
class _DpBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: AppTheme.surfaceRaised, borderRadius: BorderRadius.circular(8), border: Border.all(color: AppTheme.border, width: 0.5)),
      child: Column(children: [
        ...
        const SizedBox(height: 6),
        ClipRRect( // Progress bar is permanently in the tree to hold layout structure
          borderRadius: BorderRadius.circular(2),
          child: LinearProgressIndicator(
            value: total > 0 ? (spent / total).clamp(0.0, 1.0) : 0.0,
            backgroundColor: AppTheme.border,
            valueColor: AlwaysStoppedAnimation<Color>(spent > 0 ? (low ? AppTheme.accent : AppTheme.primary) : Colors.transparent),
            minHeight: 3,
          ),
        ),
      ]),
...
class _AttrRow extends StatelessWidget {
  ...
  final bool isSelected, canInc, canDec;
  final VoidCallback? onToggle, onIncrement, onDecrement;

  const _AttrRow({required this.attr, required this.designValue, required this.dpDelta, required this.isSelected, required this.estimGain, required this.canInc, required this.canDec, this.onToggle, this.onIncrement, this.onDecrement});

  Color get _bColor => attr.isAtLeagueMax ? AppTheme.accent : (attr.isStrength ? AppTheme.success : (attr.isWeakness ? AppTheme.error : AppTheme.primary));

  @override
  Widget build(BuildContext context) {
    final atMax = onToggle == null, hasSpnd = dpDelta > 0;
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: isSelected ? AppTheme.primary : (hasSpnd ? AppTheme.accent.withOpacity(0.5) : AppTheme.border), width: (isSelected || hasSpnd) ? 1.0 : 0.5),
      ),
      child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
        GestureDetector(
          ...
          child: Padding(
            ...
            child: AnimatedContainer(
              duration: const Duration(milliseconds: 130), width: 18, height: 18,
              decoration: BoxDecoration(shape: BoxShape.circle, color: isSelected ? AppTheme.primary : Colors.transparent, border: Border.all(color: atMax ? AppTheme.border : (isSelected ? AppTheme.primary : AppTheme.borderBright), width: 1.5)),
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              ClipRRect(
                ...
                child: SizedBox(height: 4, child: Stack(children: [
                  LinearProgressIndicator(value: attr.progress, backgroundColor: AppTheme.border, valueColor: AlwaysStoppedAnimation<Color>(_bColor)),
                  if (dpDelta > 0 && attr.leagueMax > 0) LinearProgressIndicator(value: (designValue / attr.leagueMax).clamp(0.0, 1.0), backgroundColor: Colors.transparent, valueColor: AlwaysStoppedAnimation<Color>(designValue >= attr.leagueMax ? AppTheme.accent : const Color(0xFF8B5CF6).withOpacity(0.75))),
                ])),
              ),
              const SizedBox(height: 3),
              Visibility(
                visible: estimGain > 0, maintainSize: true, maintainAnimation: true, maintainState: true,
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            SizedBox(
              ...
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Visibility(
                  visible: hasSpnd, maintainSize: true, maintainAnimation: true, maintainState: true,
...
class _StepBtn extends StatefulWidget {
  ...
  @override State<_StepBtn> createState() => _StepBtnState();
}

class _StepBtnState extends State<_StepBtn> {
  Timer? _iDly, _rTmr; bool _prs = false;
  void _onTapDown(TapDownDetails _) {
    if (widget.onTap == null) return;
    widget.onTap!(); setState(() => _prs = true);
    _iDly = Timer(const Duration(milliseconds: 400), () => _rTmr = Timer.periodic(const Duration(milliseconds: 80), (_) { if (widget.onTap != null) widget.onTap!(); }));
  }
  void _cancel() { _iDly?.cancel(); _rTmr?.cancel(); if (mounted) setState(() => _prs = false); }
  @override void dispose() { _iDly?.cancel(); _rTmr?.cancel(); super.dispose(); }

  @override
  Widget build(BuildContext context) {
    final act = widget.active && widget.onTap != null;
    return GestureDetector(
      onTapDown: act ? _onTapDown : null, onTapUp: (_) => _cancel(), onTapCancel: _cancel, onLongPressCancel: _cancel,
      child: AnimatedContainer(
        duration: const Duration(milliseconds: 100), width: 22, height: 22, alignment: Alignment.center,
        decoration: BoxDecoration(shape: BoxShape.circle, color: _prs ? AppTheme.accent.withOpacity(0.35) : (act ? AppTheme.accent.withOpacity(0.18) : AppTheme.surfaceRaised), border: Border.all(color: act ? AppTheme.accent : AppTheme.border, width: 0.8)),
...
class _Badge extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Container(
    margin: const EdgeInsets.only(right: 3), padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 1),
